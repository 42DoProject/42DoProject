...
export const getList = async (request: Request, response: Response) => {
  ...
  // ordering
  if (order === "view") inputOrder = "viewCount";
  else if (order === "like") inputOrder = "like";
  else if (order === undefined) inputOrder = "createdAt";
  else {
  ...
  if (skill !== undefined && typeof skill === "object") {
    inputSkill = [];
    for (const value of Object.values(skill)) inputSkill.push(Number(value));
    inputSkill = "[" + String(inputSkill) + "]";
  } else inputSkill = skill;
  ...
  } else inputPosition = position;
  if (state === undefined) {
    if (skill === undefined) {
      if (position === undefined) where = {};
      else
    ...
    if (skill === undefined) {
      if (position === undefined) where = { state: state };
      else
        where = sequelize.and(
...
const arrayCondition = (array: Number[], max: Number): Number[] => {
  return array.filter((item) => 0 <= item && item <= max);
};
...
export const postList = async (request: Request, response: Response) => {
  ...
  if (reference !== undefined) reference = JSON.parse(reference);
  ...
  try {
    if (position)
      position = arrayCondition(position, Number(process.env.POSITION));
  } catch (e) {
  ...
  try {
    ...
    response
      .status(200)
      .json({ newProject: project!.id, message: "added successfully." });
  } catch (error) {
    response.status(405).json({ errMessage: String(error) });
    return;
  }
...
export const updateList = async (request: Request, response: Response) => {
  ...
  try {
    ...
    if (imageLink !== undefined) {
      ...
      // input search object
      search.updateProject({ image: imageLink }, { id: Number(projectId) });
    }
    // input search object
    search.updateProject({ title: title }, { id: Number(projectId) });
...
export const getStatus = async (request: Request, response: Response) => {
  ...
  try {
    ...
    if (user1?.projectprofile !== undefined)
      response
        .status(200)
        .json({ connectProfileId: request.user!.id, status: "participating" });
    else if (user2?.applyprojectprofile !== undefined)
      response.status(200).json({
        connectProfileId: request.user!.id,
        applyingPosition: user2?.applyprojectprofile[0].position,
        status: "applying",
      });
    else
      response
        .status(200)
        .json({ connectProfileId: request.user!.id, status: "nothing" });
  } catch (error) {
    response.status(405).json({ errMessage: String(error) });
    return;
  }
...
export const contentImage = async (request: Request, response: Response) => {
  try {
    await new Promise((resolve, reject) => {
      awsS3.projectContent.single("image")(request, response, (err) => {
        if (err instanceof multer.MulterError) {
          reject(err.message);
        } else if (err) {
          reject(err.message);
...
export const postComments = async (request: Request, response: Response) => {
  ...
  try {
    ...
    const newCount: number =
      project?.commentCount === undefined ? 0 : project?.commentCount + 1;
    await Project.update(
      {
...
export const applyTeam = async (request: Request, response: Response) => {
  ...
  if (projectId === undefined && position === undefined) {
    response
      .status(400)
      .json({ errMessage: "please input projectId and position value" });
    return;
  }
...
export const cancelApply = async (request: Request, response: Response) => {
  ...
  try {
    ...
    if (!applyprojectprofile) {
      response
        .status(400)
        .json({ errMessage: "invalid projectId or profileId param" });
      return;
    }
    // check authority
    if (
      request.user!.id != profileId &&
      request.user!.id !== applyprojectprofile?.project.leader
    ) {
      response.status(401).json({ errMessage: "no authority" });
      return;
    }
    ...
    if (request.user!.id === applyprojectprofile?.project.leader)
      feed.project(
        ...
        Number(profileId),
        Number(projectId),
        applyprojectprofile?.project.title
      );
  } catch (error) {
    response.status(405).json({ errMessage: String(error) });
    return;
...
export const checkInterestProject = async (
  request: Request,
  response: Response
) => {
  const { projectId } = request.params;
  ...
  try {
    ...
    else response.status(200).json({ message: "true" });
  } catch (error) {
    response.status(405).json({ errMessage: String(error) });
    return;
  }
...
export const modifyPosition = async (request: Request, response: Response) => {
  ...
  if (
    ...
    response.status(400).json({
      errMessage: "please input projectId or profileId or position value",
    });
    return;
  }
...
export const deletePosition = async (request: Request, response: Response) => {
  ...
  if (projectId === undefined || position === undefined) {
    response
      .status(400)
      .json({ errMessage: "please input projectId or position value" });
    return;
  }
